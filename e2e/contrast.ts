 ...
 *    `color-mix()`. This page is built out of them: every `.btn`, every status
 *    `.pill`, the fault panel, the MITM verdict box, the matched/differing byte
 *    cells in the key-exchange panel, the HKDF output card and the hero aside
 *    are all `color-mix()` fills, and `body` is two `radial-gradient` washes over
 *    a `linear-gradient`. axe files every one of those nodes under `incomplete`
 *    rather than measuring it — which is most of the coloured surfaces the lab
 *    uses to teach.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This is the reason the gate
 *    that preceded this one was worthless here: the message ladder dims every
 *    not-yet-reached row to `opacity: .45`, and the old gate injected
 *    `opacity: 1 !important` before scanning, so it measured eleven rows the
 *    browser never paints that way. The hero subtitle also renders at
 *    `opacity: .85`.
 *    Note this lab collapses its animations to 0.01ms under reduced motion,
 *    which preserves end states.
 *
 ...
 *    boxes are the four `.codebox` record fields (`overflow-x: auto;
 *    white-space: pre`) and the `.deriv-bar-track`. Content scrolled past those
 *    boxes is not dimmed or partly drawn — it is absent from the frame, and
 ...
 *
 * The default is the whole page, and that is what every `scan` uses. The
 * narrowed form exists for one state this page has that a full scan cannot
 * catch reliably: the arrival flash on the receiving lane head lasts 500ms
 * (`window.setTimeout` inside `animatePacket`), which is shorter than a full
 * axe pass, so a whole-page scan would measure it only sometimes — and a
 * sometimes-measured assertion is worse than none. Scoped to `.lane-heads *`
 * the walk is a couple of milliseconds, so the gate can click Step, confirm the
 * flash class is on, measure, and confirm it is still on.
 */
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * surface on the page is authored with `color-mix()`, and Chromium reports
     ...
     *
     * A driven scan here walks eight ladder rows, the detail card, the two
     * 32-byte key-exchange grids (that alone is 64 `.byte` cells rendered
     * twice), the eight-row key schedule, the four record fields, the
     * five-row comparison table, the three scope cards and the MITM verdict
     * list, and every one of those nodes re-walks the same handful of ancestors
     * up to `<body>`. Without the caches the pass re-reads the same computed
     * styles and rects thousands of times, and the gate scans a couple of dozen
     ...
     * that is the four `.codebox` record fields and `.deriv-bar-track`.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has exactly that case: `.deriv`, the "show one
      // derivation in full" disclosure in section 5, is a closed <details>
      // holding the whole HKDF-Expand-Label walkthrough. `checkVisibility()`
      // catches it, and the gate opens it by clicking its <summary> — the route
      // a reader has — rather than setting `.open` from script.
     ...
     * it, the `aria-hidden` decoration this lab carries — the `.packet` token
     * that flies along the wire (permanently `opacity: 0` except mid-flight),
     * the `.deriv-bar-track`/`.deriv-bar-fill` length bars and the `▼ HKDF`
     * arrow — would be measured as if each took its element's declared `color`,
     * reporting ratios for marks that carry no text at all.
     */
